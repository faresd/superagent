...
var request = module.exports = require('./request').bind({}, Request);
...
Fetcher.prototype.setRequestHeader = function(field, value) {
  ...
  if(field === "Authorization") this.params.headers[field] = value;
}
...
function Response(fetcher, request) {
  this.req = request;
  var result = this.result = fetcher.result;

  this.header = this.headers = result.getAllHeaders();
  this.text = result.getContentText();
  this.setHeaderProperties(this.header);
  this.setStatusProperties(result.getResponseCode());
...
function Request(method, url) {
  ...
  this.on('end', function(){
    var res = new Response(self.fetcher, self);
    if ('HEAD' == method) res.text = null;
    self.callback(res.error, res);
  });
}
...
Request.prototype.type = function(type){
  this.setInParam('Content-Type', request.types[type] || type);
  return this;
};
